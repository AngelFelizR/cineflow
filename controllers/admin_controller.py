...
from models import Cine, Genero, Pelicula, Funcion, TipoSala, Sala, Asiento, Boleto, BoletoCancelado, BoletoUsado, TipoBoleto
...
class DashboardController:
    ...
    def obtener_opciones_filtros():
        ...
        try:
            ...
            # Tipos de sala activos
            tipos_sala = session.query(TipoSala).filter_by(Activo=True).order_by(TipoSala.Tipo).all()
            ...
            funciones = session.query(Funcion).options(
                joinedload(Funcion.sala)  # Cargar la relación sala
            ).filter(
                Funcion.FechaHora >= fecha_limite,
                Funcion.Activo == True
            ).order_by(Funcion.FechaHora.desc()).limit(100).all()
            
            # Ahora aseguramos que cada función tenga acceso a su sala
            # Si alguna función no tiene sala, le asignamos un objeto vacío
            for funcion in funciones:
                if not hasattr(funcion, 'sala') or funcion.sala is None:
                    # Creamos un objeto sala vacío con atributos por defecto
                    class SalaVacia:
                        Id = None
                        NumeroDeSala = "N/A"
                        IdCine = None
            ...
            # En caso de error, devolver listas vacías pero con estructuras válidas
            return {
                'cines': [],
                'generos': [],
                'peliculas': [],
    ...
    def obtener_ingresos(filtros: Dict[str, Any]) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            # Query SQL para ingresos
            query = text(f"""
                SELECT 
                    {agrupacion_sql} AS Periodo,
                    SUM(b.ValorPagado) AS Ingresos,
                    COUNT(b.Id) AS BoletosVendidos,
                    DATEPART(weekday, MIN(f.FechaHora)) AS DiaSemana,
                    DATEPART(week, MIN(f.FechaHora)) AS SemanaNumero,
                    DATEPART(month, MIN(f.FechaHora)) AS MesNumero
                FROM Funciones f
            ...
            for row in result:
                datos.append({
                    'Periodo': row.Periodo.isoformat() if isinstance(row.Periodo, date) else str(row.Periodo),
                    'Ingresos': float(row.Ingresos) if row.Ingresos else 0.0,
                    'BoletosVendidos': row.BoletosVendidos or 0,
                    'DiaSemana': row.DiaSemana or 0,
                    'SemanaNumero': row.SemanaNumero or 0,
                    'MesNumero': row.MesNumero or 0
                })
    ...
    def generar_pdf(datos_dashboard: Dict[str, Any], filtros: Dict[str, Any]) -> BytesIO:
        ...
        try:
            ...
            if datos_dashboard.get('ingresos'):
            ...
            if datos_dashboard.get('ocupacion'):
            ...
            if datos_dashboard.get('boletos_usados'):
            ...
            if datos_dashboard.get('cancelaciones'):
